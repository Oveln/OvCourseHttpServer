...
impl From<web::Json<CreateTeacher>> for CreateTeacher{
    fn from(create_teacher: web::Json<CreateTeacher>) -> Self {
        CreateTeacher { 
            name: create_teacher.name.clone(), 
            picture_url: create_teacher.picture_url.clone(), 
            profile: create_teacher.profile.clone()
        }
    }
}
